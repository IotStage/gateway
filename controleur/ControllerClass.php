<?php
	...
	Class Controller{
		...
		function connecteToDataBaseLampadaire($host=HOSTNAME, $basename=DATABASE_LAMPADAIRE, $username=USERNAME, $password=PASSWORD, $type_base="mysql"){
			$bdd = null;
			if($type_base == "mysql"){
	...
	//$controller->insertToDataBase($bdd_senpluvio, array($vitesse_vent, $direction_vent, $cardinalite, $temperature, $humidite, $rayonnement_solaire, $ladate));
		...
		*/
		function writeLog($event, $type_event, $plateforme=""){
			...
			if($type_event == "error"){
				$index_file = "error_" . $index_file . ".log";
				...
				$index_file = "succes_" . $index_file . ".log";
                                exec("echo \"$date_heure -> EVENT: $event\" >> /var/log/$plateforme/$index_file");
                        }
